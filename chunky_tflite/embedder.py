...
"""
ChunkySeal TFLite Embedder for high-capacity watermark embedding.

This embedder supports 1024-bit watermarks (4× the capacity of VideoSeal).

Example:
    >>> from videoseal.chunky_tflite import load_embedder
...
class ChunkySealEmbedderTFLite:
    """
    TFLite-based ChunkySeal Embedder for high-capacity watermark embedding.
    
    This embedder can:
    - Embed a 1024-bit message into an image (4× VideoSeal capacity)
    ...
    Example:
        >>> embedder = ChunkySealEmbedderTFLite("chunkyseal_embedder_tflite_256.tflite")
        >>> img = Image.open("original.jpg")
        >>> message = np.random.randint(0, 2, 1024)
        >>> img_w = embedder.embed(img, message)
        >>> img_w.save("watermarked.jpg")
    ...
    def __init__(
        ...
        self.nbits = 1024  # ChunkySeal uses 1024-bit messages
        ...
        print(f"Loaded ChunkySeal TFLite embedder: {self.model_path.name}")
    ...
    def preprocess_message(
        ...
        Returns:
            Preprocessed message as numpy array [1, 1024] with float32 values
        """
    ...
    def embed_batch(
        ...
        Args:
            ...
            messages: Array of messages [N, 1024] or list of messages
        ...
        Example:
            ...
            >>> messages = np.random.randint(0, 2, (5, 1024))
...
def load_embedder(
    ...
    """
    Load a ChunkySeal TFLite embedder model.
    
    Args:
        model_name: Model name (default: "chunkyseal")
        quantization: Quantization type - "float32", "int8", or "fp16"
        model_dir: Directory containing TFLite models
                  (default: ~/work/models/chunkyseal_tflite)
    
    Returns:
        ChunkySealEmbedderTFLite instance
    ...
    if model_dir is None:
        model_dir = Path.home() / "work" / "models" / "chunkyseal_tflite"
    else:
    ...
    if quantization == "int8":
        raise ValueError(
            "INT8 quantization is not supported for ChunkySeal embedder due to "
            "TFLite BROADCAST_TO operation limitations. Use 'float32' instead.\n\n"
            "For mobile deployment, consider:\n"
            "1. Use FLOAT32 embedder (3.9 GB)\n"
            "2. Use FP16 if available (~2 GB)\n"
    ...
    return ChunkySealEmbedderTFLite(model_path)
