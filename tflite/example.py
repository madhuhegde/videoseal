...
"""
Simple example of using VideoSeal TFLite detector.
"""
...
def main():
    print("="*70)
    print("VideoSeal TFLite Detector - Simple Example")
    print("="*70)
    
    # Example 1: Load FLOAT32 detector
    print("\n1. Loading FLOAT32 TFLite detector...")
    detector = load_detector(model_name='videoseal')
    print(f"   {detector}")
    
    # Example 2: Load INT8 detector (if available)
    print("\n2. Loading INT8 TFLite detector...")
    try:
        detector_int8 = load_detector(model_name='videoseal', quantization='int8')
    ...
    print(f"   Total '1' bits: {result['message'].sum()}/256")
    ...
    print(f"   Hex:  {message_hex[:32]}...")
    print(f"   Int:  {message_int}")
    print(f"   Bits: {message_bits[:32]}...")
